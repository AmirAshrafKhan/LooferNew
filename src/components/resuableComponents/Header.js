...
import SearchProductsCards from "./SearchProductsCards";
...
const Header = () => {
  ...
  //           // Check if data is not empty before navigating
  //           if (data.length > 0) {
        ...
        {childCategories.length > 0 ? (
          childCategories.map((childCategory) => (
            <Menu.Item
              ...
              onClick={() => getProductsById(childCategory.id)}
            >
              {childCategory.name}
            </Menu.Item>
          ))
  ...
  const menuItems = menuCategory.categoryItem.map((category) => (
    <Dropdown
      key={category.id}
      overlay={renderDropdown(category.child)}
      style={{ color: "#58595b", fontSize: 12 }}
    >
      <a
        onClick={() => getProductsById(category.id)}
        style={{ padding: "20px", color: "#58595b", fontSize: 12 }}
      >
        <Space
          style={{
            fontSize: 14,
